...
group_region_file = 'D:\\Drought_and_heat_wave_coupling\\data\\mask\\urban_civilian\\raster_filter\\'
group_mask_path_list = glob.glob(group_region_file + '*.tif')
image_file = 'D:\\Drought_and_heat_wave_coupling\\data\\urban\\'
...
for i in range(0, len(group_mask_path_list), 1):
    ...
    for j in range(0, len(image_list), 1):
        ...
        group_area = np.sum(mask_array == 1)
        group_urban = np.sum((mask_array == 1) & (image_array == 1))
        ratio = group_urban / group_area
...
saveExcel(array, 'D:\\Drought_and_heat_wave_coupling\\data\\urban\\urban_civilian.xlsx')