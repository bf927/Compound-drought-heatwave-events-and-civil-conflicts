...
for i in range(0, len(group_mask_path_list), 1):
    ...
    for j in range(0, len(landcover_list), 1):
        ...
        if rainfall_area != 0:
            rainfall_ndvi_rate = rainfall_crop_ndvi / rainfall_area
        else:
            rainfall_ndvi_rate = 0
        rainfall_list.append(rainfall_ndvi_rate)
        ...
        if irrigate_area != 0:
            irrigate_ndvi_rate = irrigate_crop_ndvi / irrigate_area
        else:
            irrigate_ndvi_rate = 0
        irrigate_list.append(irrigate_ndvi_rate)

        total_ndvi = rainfall_crop_ndvi + irrigate_crop_ndvi
        total_area = rainfall_area + irrigate_area
        if total_area != 0:
            total_rate = total_ndvi / total_area
        else:
            total_rate = 0
        total_list.append(total_rate)
...
saveExcel(array, 'D:\\Drought_and_heat_wave_coupling\\data\\cumNDVI\\cumNDVI_by_area.xlsx')
