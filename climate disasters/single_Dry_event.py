...
save_file = 'C:\\Users\\Administrator\\Desktop\\single_dry_feature.xlsx'
...
for i in range(2001, 2021, 1):
    ...
    for index in range(0, len(group_mask_path_list), 1):
        ...
        spei_encode = np.where(spei_sequence > -1, 0, 1)
        dry_encode = np.where((cdhw_encode == 1) & (spei_encode == 1), 0, spei_encode)

        duration = np.sum(dry_encode)
        ...
        output_array_part = np.concatenate([group_array, time_array, duration_array],
                                           axis=1)

output_array = np.concatenate([group_array, time_array, duration_array], axis=1)
