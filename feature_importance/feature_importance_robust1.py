...
hs_ssvi = data2['pop_dens'] * weight_array[0] + data2['imr'] * weight_array[1] + \
          ...
          data2['political_group'] * weight_array[4] - data2['Effectiveness'] * weight_array[5]

array = np.zeros((hs_ssvi.shape[0], 1))
array[:, 0] = hs_ssvi

# saveExcel(array,
#           'C:\\Users\\Administrator\\Desktop\\Drought_and_heat_wave_coupling\\data\\reg_feature\\vi_effectiveness.xlsx')

index = np.argsort(feature_importance)
new_feature = np.array([
    'Population density', 'Infant mortality rate',
    'Political exclusion', 'History conflict', 'Political group', 'Government effectiveness'])[index]
